...
![Where the whisky flavor profile data came from | R-bloggers](https://revolution-computing.typepad.com/.a/6a010534b1db25970b01a3fc385b47970b-800wi)
...
- 중심 경향성의 측도 (central tendency) : 
  - 평균 (sample mean) : 특이값에 의해 망가지기 쉬움
  - 중앙값 (median) : 매우 로버스트한 방법임
  - 최빈값 (mode)
- 산포도의 측도 (dispersion) 
  - 범위 (range)
  - 분산 (variance)
...
- Five-Number Summary : they make *box plot*
  - 최소값
  - 1분위수
  - 중앙값
  - 3분위수
...
- 절차
  ...
  3. 검정통계량의 값을 유의수준과 비교하여 평가한다. 
     ...
     이때, 유의확률이나 기각역을 통해 검정통계량의 값을 평가하고 귀무가설을 기각할 수 있는지 판단한다. (검정통계량의 분포를 알아야 하는데 표본평균이면 대충 정규분포)
...
검정통계량 ~ t(n-2) 따른다.   ** 아래 식은 확인이 필요함!!! **
$$
T = sqrt(n-2)* r / sqrt(1-r^2)
$$

- Pearson's correlation ***r*** : 두 변량이 정규모집단으로부터 랜덤표본일 때

- Spearman's $\rho$: ...이 아닐 때
...
- 할일
  ...
  2. 기울기=0이란 귀무가설을 검정 (ie. $H_0$: $\beta_1=0 $)
...
- 따라서, 선형모형의 잔차도 바로 정규분포가 될 것이 기대됨 
- 잔차분석 :  분석을 마쳤을 때 잔차가 정규분포와 흡사하면 올바른 분석을 했다고 안심하면 됨. 
...
- 100년 역사 
- 다변량 숫자변수를 행렬대수 기법을 이용하여 차원을 낮추는 기술
...
1. 주어진 자료를 본다
 	...
 	3. 산포가 최대가 되는 방향을 찾는다. (즉, 자료를 가장 잘 설명하는 직선을 긋는다)
 	4. 수직을 찾는다
 	5. 3, 4가 새로운 축이 된다.
